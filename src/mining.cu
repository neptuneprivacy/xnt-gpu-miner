...
UnifiedMiningController::UnifiedMiningController(
    ...
    : rpc_url(rpc_url), gpu_id(gpu_id), gpu_resources(resources)
{
}
...
bool UnifiedMiningController::reconnect_to_node()
{
    ...
    while (!connected && !stop_mining && !gpu_resources->gpu_stop_flag && attempt < MAX_RECONNECT_ATTEMPTS)
    {
        ...
        if (stop_mining || gpu_resources->gpu_stop_flag)
            break;
...
void UnifiedMiningController::handleNewPuzzle(const MiningEvent &event)
{
    ...
    if (need_preprocess)
    {
        // CRITICAL: Free old buffer BEFORE preprocessing to avoid memory overlap.
        // Without this, we'd have old + new buffers allocated simultaneously,
        // causing ~160MB memory pressure and temp_leafs allocation failure.
        {
            std::lock_guard<std::mutex> lock(gpu_resources->state_mutex);
            if (gpu_resources->buffer)
            {
                gpu_resources->buffer->cleanup();
                gpu_resources->buffer.reset();
            }
        }
...
MultiGpuManager::MultiGpuManager(
    const std::string &rpc_url,
    int specific_gpu)
    : rpc_url(rpc_url), single_gpu_id(specific_gpu)
{
}
